...
#pragma DATA_SECTION( ping_2  , "DMARAML4" )
#pragma DATA_SECTION( pong_2  , "DMARAML4" )
#pragma DATA_SECTION( garbage , "DMARAML5" )
#pragma DATA_SECTION( audio   , "DMARAML5" )
...
interrupt void DMA_CH2(void)
{
    while(ready)
    {
        ...
        if(program_state == 0)
        {
            rfft.InBuf     = (float*)&pong_1[0];       //Input buffer
        ...
        {
            rfft.InBuf     = (float*)&ping_1[0];       //Input buffer
...
void Init_Timers(void)
{
    ...
    ConfigCpuTimer( &CpuTimer2 , 150 , 1000000);  //5800 values in 1 second
    ConfigCpuTimer( &CpuTimer1 , 150 , 22);   //45454Hz sampling frequency

    PieVectTable.TINT2 = LCD_ISR;
...
void Init_DMA(void)
{
    ...
    EALLOW;
    PieCtrlRegs.PIEIER7.bit.INTx1 = 1; //enable DMACH1 RRDY int
